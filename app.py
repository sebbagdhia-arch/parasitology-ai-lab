...
#  2. الثوابت
# ============================================
APP_VERSION = "4.1.0"  # ✅ إصدار جديد بعد إصلاح النموذج
APP_PASSWORD = "123"
...
# ============================================
#  3. ✅ ترتيب الأصناف حسب labels.txt بالضبط
#     هذا هو الإصلاح الأهم!
# ============================================
CLASS_NAMES_ORDERED = [
    "Amoeba (E. histolytica)",   # 0
    "Giardia",                    # 1
    "Leishmania",                 # 2
    "Plasmodium",                 # 3
    "Trypanosoma",                # 4
...
def load_labels(filepath="labels.txt"):
    """✅ تحميل labels.txt من Teachable Machine"""
    labels = []
    if os.path.exists(filepath):
        with open(filepath, "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
                if line:
                    # Teachable Machine format: "0 Amoeba" أو "Amoeba"
                    parts = line.split(" ", 1)
...
TRANSLATIONS = {
    "fr": {
        ...
        "scan_demo_mode": "⚠️ Mode démonstration — Aucun modèle IA détecté ! Placez votre fichier model (keras_model.h5 ou model.tflite) dans le même dossier.",
        "scan_real_mode": "✅ Modèle IA chargé — Résultats réels",
    ...
    "ar": {
        ...
        "scan_demo_mode": "⚠️ وضع عرض — لم يتم الكشف عن نموذج ذكاء اصطناعي! ضع ملف النموذج في نفس المجلد.",
        "scan_real_mode": "✅ تم تحميل نموذج الذكاء الاصطناعي — نتائج حقيقية",
    ...
    "en": {
        ...
        "scan_demo_mode": "⚠️ Demo mode — No AI model detected! Place your model file in the same folder.",
        "scan_real_mode": "✅ AI Model loaded — Real results",
...
def generate_heatmap_overlay(image, seed=None):
    ...
    if seed is None:
        seed = st.session_state.get("heatmap_seed", random.randint(0, 999999))
...
# ============================================
#  10. ✅ محرك الذكاء الاصطناعي - الإصلاح الرئيسي
# ============================================
@st.cache_resource(show_spinner=False)
def load_ai_model():
    """
    ✅ تحميل نموذج Teachable Machine بشكل صحيح
    يبحث عن:
      1. keras_model.h5 (الأكثر شيوعاً من Teachable Machine)
      2. أي ملف .h5
      3. أي ملف .keras
      4. model.savedmodel (مجلد)
      5. أي ملف .tflite
    """
    model = None
    model_name = None
    model_type = None  # "keras" أو "tflite"
    ...
    try:
        ...
        # ✅ الأولوية 1: keras_model.h5 (Teachable Machine الافتراضي)
        if "keras_model.h5" in files:
            try:
                model = tf.keras.models.load_model("keras_model.h5", compile=False)
                model_name = "keras_model.h5"
                model_type = "keras"
                # ✅ طباعة معلومات النموذج للتأكد
                print(f"✅ Model loaded: {model_name}")
                print(f"   Input shape: {model.input_shape}")
                print(f"   Output shape: {model.output_shape}")
            except Exception as e:
                print(f"❌ Error loading keras_model.h5: {e}")

        # ✅ الأولوية 2: أي ملف .h5 آخر
        if model is None:
            h5_files = [f for f in files if f.endswith(".h5")]
            for h5f in h5_files:
                try:
                    model = tf.keras.models.load_model(h5f, compile=False)
                    model_name = h5f
                    model_type = "keras"
                    print(f"✅ Model loaded: {model_name}")
                    break
                except Exception as e:
                    print(f"⚠️ Failed to load {h5f}: {e}")

        # ✅ الأولوية 3: ملف .keras
        if model is None:
        ...
        if model is None:
            ...
            for tlf in tflite_files:
                try:
                    interpreter = tf.lite.Interpreter(model_path=tlf)
...
def get_class_names():
    """✅ تحميل أسماء الأصناف من labels.txt أو استخدام الترتيب الافتراضي"""
    # أولاً: حاول تحميل من labels.txt
    loaded = load_labels("labels.txt")
    if loaded:
        # ✅ ربط الأسماء المختصرة من labels.txt بأسماء PARASITE_DB
        mapped = []
        for label in loaded:
            label_lower = label.lower().strip()
            found = False
            for db_name in PARASITE_DB.keys():
                if label_lower in db_name.lower() or db_name.lower().startswith(label_lower):
                    mapped.append(db_name)
                    found = True
                    break
            if not found:
                # إذا لم يوجد تطابق، ابحث بشكل أدق
                for db_name in PARASITE_DB.keys():
                    if label_lower.split()[0] in db_name.lower():
                        mapped.append(db_name)
                        found = True
                        break
            if not found:
                mapped.append(label)  # استخدم الاسم كما هو
        print(f"✅ Labels loaded from labels.txt: {mapped}")
        return mapped

    # ثانياً: استخدم الترتيب الافتراضي
    print("⚠️ labels.txt not found, using default order")
    return CLASS_NAMES_ORDERED


def preprocess_image_for_teachable_machine(image):
    """
    ✅ معالجة الصورة بنفس طريقة Teachable Machine بالضبط
    Teachable Machine يستخدم:
    - حجم 224x224
    - تطبيع: (pixel / 127.5) - 1.0 → المجال [-1, 1]
    - ملء مربع (لا تمديد)
    """
    # ✅ تحويل لـ RGB
    img = image.convert("RGB")

    # ✅ تغيير الحجم مع الحفاظ على النسبة (مثل Teachable Machine)
    img = ImageOps.fit(img, MODEL_INPUT_SIZE, Image.LANCZOS)

    # ✅ تحويل لمصفوفة NumPy
    img_array = np.asarray(img, dtype=np.float32)

    # ✅ التطبيع بنفس طريقة Teachable Machine
    # الطريقة: (pixel_value / 127.5) - 1.0
...
def predict_image(model, model_type, image, class_names):
    """
    ✅ التنبؤ الحقيقي بالصورة — الإصلاح الرئيسي
    """
    result = {
    ...
    # ═══════════════════════════════════════════
    # ✅ إذا لا يوجد نموذج → وضع ديمو
    # ═══════════════════════════════════════════
    if model is None:
        rng = random.Random(hash(image.tobytes()) % 2**32)
        demo_label = rng.choice(class_names)
        demo_conf = rng.randint(55, 95)

        all_preds = {}
        remaining = 100.0 - demo_conf
        others = [c for c in class_names if c != demo_label]
        for cls in others:
            val = round(rng.uniform(0, remaining / max(1, len(others))), 1)
            all_preds[cls] = val
        all_preds[demo_label] = float(demo_conf)
    ...
    # ═══════════════════════════════════════════
    # ✅ التنبؤ الحقيقي بالنموذج
    # ═══════════════════════════════════════════
    try:
        import tensorflow as tf

        # ✅ معالجة الصورة بنفس طريقة Teachable Machine
        batch = preprocess_image_for_teachable_machine(image)

        if model_type == "tflite":
            # ═══ TFLite ═══
            inp_details = model.get_input_details()
            out_details = model.get_output_details()

            # التأكد من نوع البيانات المطلوب
            input_dtype = inp_details[0]['dtype']
            if input_dtype == np.uint8:
                # بعض نماذج TFLite تحتاج uint8
                batch_uint8 = ((batch + 1.0) * 127.5).astype(np.uint8)
                model.set_tensor(inp_details[0]['index'], batch_uint8)
            else:
                model.set_tensor(inp_details[0]['index'], batch.astype(input_dtype))

            model.invoke()
            preds = model.get_tensor(out_details[0]['index'])[0]

        else:
            # ═══ Keras Model ═══
            preds = model.predict(batch, verbose=0)[0]

        # ✅ التأكد من أن المخرجات صحيحة
        print(f"🔍 Raw predictions: {preds}")
        print(f"🔍 Sum of predictions: {np.sum(preds)}")
        print(f"🔍 Number of classes: {len(preds)} vs {len(class_names)}")

        # ✅ إذا المخرجات ليست احتمالات (مجموعها ≠ 1)، طبّق softmax
        if np.sum(preds) < 0.99 or np.sum(preds) > 1.01:
            # قد تكون logits وليست probabilities
            exp_preds = np.exp(preds - np.max(preds))
            preds = exp_preds / np.sum(exp_preds)
            print(f"🔄 Applied softmax: {preds}")

        # ✅ استخراج النتيجة
        num_classes = min(len(preds), len(class_names))
        idx = int(np.argmax(preds[:num_classes]))
...
# ============================================
#  13. شاشة التحميل + القفل التلقائي
# ============================================
...
# ╚══════════════════════════════════╝
    ...
    # ✅ عرض حالة النموذج بوضوح
    if model is not None:
        st.sidebar.success(f"🧠 {model_name} ({model_type})")
        st.success(t("scan_real_mode"))
    else:
        st.sidebar.warning("🧠 Demo Mode")
        st.warning(t("scan_demo_mode"))

    # ✅ معلومات تقنية في expander
    with st.expander("🔧 Debug: Model Info"):
        st.write(f"**Model loaded:** {model is not None}")
        st.write(f"**Model name:** {model_name}")
        st.write(f"**Model type:** {model_type}")
        st.write(f"**Class names ({len(class_names)}):** {class_names}")
    ...
    if img_file is not None:
        ...
        with col_res:
            ...
            with st.spinner(f"⏳ {t('scan_analyzing')}"):
                ...
                # ✅ التنبؤ الحقيقي
                result = predict_image(model, model_type, image, class_names)
            ...
            # ✅ توضيح واضح إذا كان وضع ديمو أو حقيقي
            if result["is_demo"]:
                st.error(f"🎲 {t('scan_demo_mode')}")
            else:
                st.success(f"✅ {t('scan_real_mode')}")
            ...
            if result["all_predictions"]:
                with st.expander(f"📊 {t('scan_all_probs')}"):
                    for cls, prob in sorted(result["all_predictions"].items(), key=lambda x: x[1], reverse=True):
                        bar_color = "#22c55e" if prob == max(result["all_predictions"].values()) else "#64748b"
        ...
        with a2:
            if st.button(f"💾 {t('scan_save')}", use_container_width=True):
                entry = {
                    ...
                    "Mode": "IA" if not result["is_demo"] else "Demo"
                }
                st.session_state.history.append(entry)
                log_activity(f"Scan saved: {label} ({conf}%) for {p_nom} [{'Real' if not result['is_demo'] else 'Demo'}]")
...
# ╚══════════════════════════════════╝
    ...
    features = [
        ...
        ("✅", "Real AI", "Teachable Machine compatible"),
    ]
